...
def GenKeyNodes(data_dir):
    ...
    for i in name_list:
        ...
        # Extract the top 30 nodes with the highest eigenvector centrality
...
def MonCentrality(data_dir):
    ...
    for i in name_list:
        ...
        max_weight_degree = max(
            [sum([G[node][neighbor]['weight'] for neighbor in G.neighbors(node)]) for node in G.nodes])

        weigh_dc = {}
        for node in G.nodes:
            weighted_degree = sum([G[node][neighbor]['weight'] for neighbor in G.neighbors(node)])
            weigh_dc[node] = weighted_degree / max_weight_degree

        DC = M_cal(sorted(weigh_dc.items(), key=lambda x: x[1], reverse=True))
        ...
        CC = M_cal(
            sorted(nx.closeness_centrality(G, distance='weight').items(), key=lambda x: x[1], reverse=True)
        )
        BC = M_cal(
            sorted(
